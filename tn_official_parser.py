...
def parse_hospital_row(row_data):
  ...
  # 19 Misc
  resources = [
    Resource(ResourceType.BEDS, "covid_beds", int(columns[4].text), int(columns[2].text)),
    Resource(ResourceType.BED_WITH_OXYGEN, "", int(columns[7].text), int(columns[5].text)),
    Resource(ResourceType.BED_WITHOUT_OXYGEN, "", int(columns[10].text), int(columns[8].text)),
    Resource(ResourceType.ICU_WITHOUT_VENTILATOR, "", int(columns[13].text), int(columns[11].text)),
    Resource(ResourceType.ICU_WITH_VENTILATOR, "", int(columns[16].text), int(columns[14].text)),
  ]
  return Hospital(columns[0].text, "", columns[1].text, "", "Tamil Nadu", "", parser.parse(columns[17].text+ " +05:30"),
                  resources, "", TN_URL)

def parse_web_data(soup):
  hospitals = []
  hospitals_data = soup.find('table', {'id' : 'dtBasicExample'}).find('tbody').find_all('tr')
