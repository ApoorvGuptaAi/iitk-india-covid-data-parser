...
def parse_hospitals_data(page_data):
  hospitals = json.loads(page_data.string[len("var gnctd_covid_facilities_data = "):-1])
  return hospitals
...
def parse_hospital_resources(resources_details):
  ...
  for key, value in resources_details.items():
    ...
    for hospital_name, details in value.items():
      ...
      if hospitals[hospital_name].last_updated == None and 'last_udpated_at' in details:
        hospitals[hospital_name].last_updated = parser.parse(details['last_updated_at'] + "IST", tzinfos=tzinfos)
  return hospitals
