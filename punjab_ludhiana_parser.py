...
def parse_hospital_row(data):
  ...
  last_updated_at = data.find('small', {'class' : 'text-muted'}).text[len("Update On : "):]
  ...
  resources = [
    Resource(ResourceType.BED_WITHOUT_OXYGEN, "", int(bed_wo_oxy[2].text), int(bed_wo_oxy[1].text)),
    Resource(ResourceType.BED_WITH_OXYGEN, "", int(bed_oxy[2].text), int(bed_oxy[1].text)),
    Resource(ResourceType.ICU_WITH_VENTILATOR, "", int(icu_ventilator[2].text), int(icu_ventilator[1].text)),
    Resource(ResourceType.ICU_WITHOUT_VENTILATOR, "", int(icu_wo_ventilator[2].text), int(icu_wo_ventilator[1].text))
  ]
  return Hospital(hospital_name, "", "", "Ludhiana", "Punjab", "", parser.parse(last_updated_at + "+05:30"), resources, "", LUDHIANA_URL)

def parse_web_data(soup):
  hospitals = []
  hospitals_data = soup.find('div', {'class' : 'card-body'}).find_all('div', {'class': 'py-2'})
