...
def parse_hospital_row(row_data):
  ...
  availableGenBeds = int(row_data.find('span', {'id' : 'Lbloccupiedgenralbeds'}).text)
  totalGenBeds = int(row_data.find('span', {'id' : 'lbltotGenralbeds'}).text)
  availableOxyBeds = int(row_data.find('span', {'id' : 'lbloccupiedoxygenbeds'}).text)
  totalOxyBeds = int(row_data.find('span', {'id' : 'lbltotoxygenbeds'}).text)
  availableICUBeds = int(row_data.find('span', {'id' : 'lbloccupiedicubeds'}).text)
  totalICUBeds = int(row_data.find('span', {'id' : 'lbltoticubeds'}).text)
  lastUpdatedAt = parser.parse(row_data.find('span', {'id' : 'lbllastupdated'}).text + "IST", tzinfos=tzinfos)
  resources = [
               Resource(ResourceType.BED_WITHOUT_OXYGEN, "", availableGenBeds, totalGenBeds),
               Resource(ResourceType.BED_WITH_OXYGEN, "", availableOxyBeds, totalOxyBeds),
               Resource(ResourceType.ICUS, "", availableICUBeds, totalICUBeds),
  ]
  hospital = Hospital(
      hospitalName, "", district, "", "Uttarakhand", "", lastUpdatedAt,
      resources, "", UTTARAKHAND_URL, 0)
  return hospital
